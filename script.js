...
function addNewRow() {
    ...
    newRow.innerHTML = `
          ...
          <td>
            <input type="number" placeholder="01" class="qty-data mt-3" onkeyup="getSubTotal()" required />
          </td>
          <td>
...
// calculate totals
function calculateTotal() {
    var subTotal = parseFloat(document.getElementById("sub_total").innerText.replace("₹", "")) || 0;
    var taxRate = parseFloat(document.getElementById("input-tax").value) || 0;
    var discountRate = parseFloat(document.getElementById("input-discount").value) || 0;
    ...
    document.getElementById("net-amount").innerText = "₹ " + totalAmount.toFixed(2);
}
...
function openReviewModal() {
    ...
    rows.forEach(function (row, index) {
        const itemName = row.querySelector(".input-name").value;
        ...
        const price = parseFloat(row.querySelector(".rupe-text").value.replace("₹", "")) || 0;
    ...
    document.getElementById("modalInvoiceFromEmail").innerText = invoiceFrom.email;
    document.getElementById("modalInvoiceFromAddress").innerText = invoiceFrom.address;

    document.getElementById("modalInvoiceToName").innerText = invoiceTo.name;
    document.getElementsByClassName("modalInvoiceToName")[0].innerText = invoiceTo.name;
    document.getElementById("modalInvoiceToEmail").innerText = invoiceTo.email;
    document.getElementById("modalInvoiceToAddress").innerText = invoiceTo.address;
    ...
    itemDetails.forEach(item => {
        ...
        itemElement.innerHTML = `
            <p class="pt-4 mb-1"><b>Item ${item.index}:</b></p>

            <div class="d-flex justify-content-between">
              <div><b>Name:</b> <div>${item.itemName}</div></div>
              <div><b>Description:</b> <div>${item.itemDescription}</div></div>
              <div><b>Quantity:</b> <div>${item.quantity}</div></div>
              <div><b>Price:</b> <div>₹ ${item.price}</div></div>
              <div><b>Total:</b> <div>₹ ${item.total}</div></div>
            </div>
        `;
